...
int organizarTorre(TORRE *torre, int disco) {
    if ((*torre).topo >= MAXIMODEDISCOS - 1) return 0;
...
int jogoConcluido(TORRE *torreFinal, int numeroDiscos) {
    if ((*torreFinal).topo + 1 != numeroDiscos) return 0;
    for (int i = 0; i < numeroDiscos; i++) {
        if ((*torreFinal).discos[i] != numeroDiscos - i) return 0;
    }
    return 1;
}
